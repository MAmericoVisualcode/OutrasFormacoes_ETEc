...
class FormacaoAcademicaDAO {
    ...
    public function buscarPorId(int $id): ?FormacaoAcademica {
        try {
            ...
            $stmt->bindValue(':id', $id);
            $stmt->execute();
            $stmt->setFetchMode(PDO::FETCH_CLASS, 'FormacaoAcademica');
            return $stmt->fetch();
        } catch (PDOException $e) {
    ...
    public function listarPorUsuario(int $idusuario): array {
        try {
            $stmt = $this->pdo->prepare("SELECT * FROM formacaoacademica WHERE idusuario = :idusuario");
            $stmt->bindValue(':idusuario', $idusuario);
            $stmt->execute();
            return $stmt->fetchAll(PDO::FETCH_CLASS, 'FormacaoAcademica');
        } catch (PDOException $e) {
